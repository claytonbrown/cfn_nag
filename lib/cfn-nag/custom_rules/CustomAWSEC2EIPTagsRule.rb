...
class CustomAWSEC2EIPTagsRule < BaseRule
  ...
  def audit_impl(cfn_model)
    violating_resources = cfn_model.resources_by_type('AWS::EC2::EIP').select do |resource|
      resource.tags.nil?
    end
