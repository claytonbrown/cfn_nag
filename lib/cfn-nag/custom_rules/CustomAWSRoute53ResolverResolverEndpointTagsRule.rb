...
class CustomAWSRoute53ResolverResolverEndpointTagsRule < BaseRule
  def rule_text
  ...
  def audit_impl(cfn_model)
    violating_resources = cfn_model.resources_by_type('AWS::Route53Resolver::ResolverEndpoint').select do |resource|
      resource.tags.nil?
    end
